<template>
  <section>
    <b-container>
      ...
      <b-card header="User Detial" style="width: 50vw; margin:auto" v-if="isSearched">
        <b-card-body>
          <b-form>
      ...
      <form ref="form">
        ...
        <b-form-group label="Wallet Amount">
          ...
          <b-row class="my-1" style="padding: 1% 4%">
            <b-button pill block size="lg" variant="primary" @click="ViewWalletTransactions">View Transactions</b-button>
          </b-row>
      </form>
    </b-modal>
  </section>
...
export default {
  ...
  methods: {
    ...
    walletAddressInfo(address) {
      ...
      that.getWalletBalance(data).then((res) => {
        that.toggelLoader();
    ...
    ViewWalletTransactions(){
      let that = this
      that.$router.push(`/transactions?walletAddr=${that.targetWallet.address}`)
    },
    transferAmount(){
      let that = this
